...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li id="comment-<?php comment_ID() ?>" class="<?php echo $oddcomment; /* Style differently if comment author is blog author */ if ($comment->comment_author_email == get_the_author_email()) { echo ' authorcomment'; } ?>">
			...
			<p class="cmtinfo"><span class="number"><?php echo $commentnumber; $commentnumber++;?></span> <small class="commentmetadata"></small><cite><?php comment_author_link() ?></cite><em>on <?php comment_date('d M Y') ?> at <?php comment_time() ?> <a href="#comment-<?php comment_ID() ?>" title="link to this comment">link to this comment</a> <?php edit_comment_link('edit this','',''); ?></em></p>			
			<?php if ($comment->comment_approved == '0') : ?>
			<em>Dein Kommentar wird von einem Moderator geprüft.</em>
			<?php endif; ?>      
			<div class="comment">				
				<?php comment_text() ?>
			</div>		
 ...
 <?php else : // this is displayed if there are no comments so far ?>
  ...
  <?php if ('open' == $post-> comment_status) : ?> 
	 ...
	 <?php else : // comments are closed ?>
		...
		<p class="nocomments">Zur Zeit können leider keine Kommentare geschrieben werden.</p>
